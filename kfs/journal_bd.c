...
static uint32_t journal_bd_lookup_block(BD_t * object, bdesc_t * block, uint32_t block_number)
{
	...
	if(!number)
	{
		...
		if(blocks && !last)
		{
			...
			bdesc_t * record = CALL(info->journal, synthetic_read_block, record_number, object->blocksize);
			if(!record)
		...
		if(last % npb)
			number_block = CALL(info->journal, read_block, number_block_number, object->blocksize);
		else
			number_block = CALL(info->journal, synthetic_read_block, number_block_number, object->blocksize);
		assert(number_block);
...
static int journal_bd_stop_transaction(BD_t * object)
{
	...
	block = CALL(info->journal, read_block, block_number, object->blocksize);
	if(!block)
	{
...
static int journal_bd_write_block(BD_t * object, bdesc_t * block, uint32_t block_number)
{
	...
	assert(block_number + block->length / object->blocksize <= object->numblocks);
	...
	while ((chdesc = block->new_changes))
	{
		...
		while(*deps)
		{
			...
			if(dep == info->hold || (dep->block && dep->block == block))
			{
...
static int replay_single_transaction(BD_t * bd, uint32_t transaction_start, uint16_t expected_type)
{
	...
	for(block = 0; block < cr->nblocks; block += bnpb)
	{
		...
		for(index = 0; index != max; index++)
		{
			...
			head = NULL;
			r = chdesc_create_full(output, info->below_bd, data_block->data, &head);
			if(r < 0)
				goto output_error;
			r = chdesc_add_depend(info->data, head);
			if(r < 0)
				goto chdesc_error;
			r = CALL(info->below_bd, write_block, output, numbers[index]);
			if(r < 0)
				goto chdesc_error;
...
static int replay_journal(BD_t * bd)
{
	...
	for(transaction = 0; transaction < info->cr_count; transaction++)
	{
		...
		bdesc_t * commit_block = CALL(info->journal, read_block, commit_block_number, bd->blocksize);
