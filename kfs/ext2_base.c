...
static int ext2_write_block_bitmap(LFS_t * object, uint32_t blockno, bool value, chdesc_t ** head)
{
	...
	r = CALL(info->ubd, write_block, info->bitmap_cache, info->bitmap_cache->b_number);
	if(r < 0)
		return r;
...
static int ext2_write_inode_bitmap(LFS_t * object, inode_t inode_no, bool value, chdesc_t ** head)
{
	...
	r = CALL(info->ubd, write_block, info->inode_cache, info->inode_cache->b_number);
	if (r < 0)
		return r;
...
static int ext2_append_file_block_set(LFS_t * object, fdesc_t * file, uint32_t block, chdesc_t ** tail, chdesc_pass_set_t * befores)
{
	...
	else if(nblocks < EXT2_NDIRECT + n_per_block)
	{
		...
		r = CALL(info->ubd, write_block, indirect, indirect->b_number);
		if (r < 0)
			return r;
	}
	else
	{
		...
		if(!(nblocks % n_per_block))
		{
			...
			r = CALL(info->ubd, write_block, dindirect, dindirect->b_number);
			if(r < 0)
				return r;
		}
	}
                                        ...
                                        chdesc_t ** tail, chdesc_pass_set_t * befores)
{
	...
	if (basep % object->blocksize + exists_rec_len_actual + new_rec_len_actual <= object->blocksize) {
		...
		r = CALL(info->ubd, write_block, dirblock, dirblock->b_number);
		if (r < 0)
			return r;
	} else
                                    ...
                                    inode_t * new_ino, chdesc_t ** head)
{
	...
	if (!ln) {
		...
		if (type == TYPE_SYMLINK) {
			...
			r = CALL(info->ubd, write_block, dirblock_bdesc, dirblock_bdesc->b_number);
...
static uint32_t ext2_erase_block_ptr(LFS_t * object, EXT2_inode_t * inode, chdesc_t ** head)
{
	...
	else if (blocknum < EXT2_NDIRECT + n_per_block)
	{
		...
		if (blocknum == 0)
		{
			...
			//if (r < 0)
			//	return INVALID_BLOCK;
			//r = CALL(info->ubd, write_block, block_desc, block_desc->b_number);
			//if (r < 0)
			//	return INVALID_BLOCK;
		}
	}
	else if (blocknum < EXT2_NDIRECT + n_per_block + n_per_block * n_per_block)
	{
		...
		else
		{
			...
			//if (r < 0)
			//	return INVALID_BLOCK;
			//r = CALL(info->ubd, write_block, double_block_desc, double_block_desc->b_number);
			//if (r < 0)
			//	return INVALID_BLOCK;
		}
	}
...
static int ext2_delete_dirent(LFS_t * object, ext2_fdesc_t * dir_file, uint32_t basep, uint32_t prev_basep, chdesc_t ** head)
{
	...
	if(basep % object->blocksize == 0) {
		...
		return CALL(info->ubd, write_block, dirblock, dirblock->b_number);
	}
...
static int ext2_write_slow_symlink(LFS_t * object, ext2_fdesc_t * f, char * name, uint32_t name_len, chdesc_t ** head)
{
	...
	return CALL(info->ubd, write_block, new_block, new_block->b_number);
}
...
int ext2_write_inode_set(struct ext2_info * info, inode_t ino, EXT2_inode_t * inode, chdesc_t ** tail, chdesc_pass_set_t * befores)
{
	...
	if (*tail && r > 0)
	{
		...
		r = CALL(info->ubd, write_block, bdesc, bdesc->b_number);
	}
...
static int ext2_super_report(LFS_t * lfs, uint32_t group, int32_t blocks, int32_t inodes, int32_t dirs)
{
	...
	if (head && r > 0)
	{
		...
		r = CALL(info->ubd, write_block, info->super_cache, info->super_cache->b_number);
		if (r < 0)
			return r;
	}
	...
	if (head && r > 0)
	{
		...
		lfs_add_fork_head(head);
		r = CALL(info->ubd, write_block, info->gdescs[group_bdesc], info->gdescs[group_bdesc]->b_number);
	}
...
static int ext2_load_super(LFS_t * lfs)
{
	...
	info->super_cache = CALL(info->ubd, read_block, 0, info->ubd->blocksize);
	if (info->super_cache == NULL)
	{
	...
	for(i = 0; i < ngroupblocks; i++) {
		info->gdescs[i] = CALL(info->ubd, read_block, (block + i), info->lfs.blocksize);
		if(!info->gdescs[i])
			goto wb_fail2;
