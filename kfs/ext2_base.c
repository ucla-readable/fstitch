...
static int ext2_find_free_block(LFS_t * object, uint32_t * blockno)
{
	...
	do {
		...
		if(info->gnum != block_group || !info->bitmap_cache)
		{
			...
			bitmap = CALL(info->ubd, read_block, info->groups[block_group].bg_block_bitmap, info->lfs.blocksize);
			if(!bitmap)
				return -ENOENT;
			bdesc_retain(bitmap);
...
static int ext2_read_block_bitmap(LFS_t * object, uint32_t blockno)
{
	...
	if(info->gnum != block_group || !info->bitmap_cache)
	{
		if(info->bitmap_cache)
			bdesc_release(&info->bitmap_cache);	
		info->gnum = block_group;
		info->bitmap_cache = CALL(info->ubd, read_block, info->groups[block_group].bg_block_bitmap, info->lfs.blocksize);
		if(!info->bitmap_cache)
			return -ENOENT;
...
static int ext2_write_inode_bitmap(LFS_t * object, inode_t inode_no, bool value, chdesc_t ** head)
{
	...
	if(info->inode_gdesc != block_group || !info->inode_cache)
	{
		...
		info->inode_cache = CALL(info->ubd, read_block, info->groups[block_group].bg_inode_bitmap, info->lfs.blocksize);
		if(!info->inode_cache)
			return -ENOENT;
                      ...
                      uint32_t * pbasep, uint32_t * ppbasep, ext2_fdesc_t ** file)
{
	...
	for(;;)
	{
		...
		dirblock1 = CALL(info->ubd, read_block, file_blockno1, info->lfs.blocksize);
		if(!dirblock1)
			goto dir_lookup_error;

		while((*basep / object->blocksize) == blockno)
		{
			*ppbasep = *pbasep;
			*pbasep = *basep;
			...
			uint16_t rec_len;
			if(object->blocksize - block_offset >= 6)
			{
				rec_len = *((uint16_t *) (dirblock1->data + block_offset + 4));
			...
			if(rec_len == 0 || block_offset + rec_len > object->blocksize)
			{
				...
				dirblock2 = CALL(info->ubd, read_block, file_blockno2, info->lfs.blocksize);
				if(!dirblock2)
				...
				block1_len = object->blocksize - block_offset;
				...
				memcpy(&entry, dirblock1->data + block_offset, block1_len);
				memcpy((uint8_t*)(&entry) + block1_len, dirblock2->data, block2_len);
				*basep += entry.rec_len;
			...
			{
				EXT2_Dir_entry_t * pdirent = (EXT2_Dir_entry_t *) (dirblock1->data + block_offset);
...
static uint32_t get_file_block(LFS_t * object, ext2_fdesc_t * file, uint32_t offset)
{
	...
	else if (blocknum >= n_per_block + EXT2_NDIRECT)
	{
		...
		block_desc = (CALL(info->ubd, read_block, file->f_inode.i_block[EXT2_DINDIRECT], info->lfs.blocksize));
		if (!block_desc)
		{
		...
		block_desc = CALL(info->ubd, read_block, blockno, info->lfs.blocksize);
		if (!block_desc)
		{
	...
	else if (blocknum >= EXT2_NDIRECT)
	{
		...
		block_desc = (CALL(info->ubd, read_block, file->f_inode.i_block[EXT2_INDIRECT], info->lfs.blocksize));
		if (!block_desc)
		{
			Dprintf("failed indirect block lookup in %s\n", __FUNCTION__);
			return INVALID_BLOCK;
...
static int ext2_get_disk_dirent(LFS_t * object, ext2_fdesc_t * file, uint32_t * basep, const EXT2_Dir_entry_t ** dirent)
{
	...
	dirblock = CALL(info->ubd, read_block, file_blockno, info->lfs.blocksize);
	if (!dirblock)
		return -1;
...
static int ext2_append_file_block_set(LFS_t * object, fdesc_t * file, uint32_t block, chdesc_t ** tail, chdesc_pass_set_t * befores)
{
	...
	else if(nblocks < EXT2_NDIRECT + n_per_block)
	{
		...
		if(!nblocks)
		{
			...
			f->f_inode.i_block[EXT2_INDIRECT] = indirect_number;
		}
		else
		{
			int offset = nblocks * sizeof(uint32_t);
			indirect_number = f->f_inode.i_block[EXT2_INDIRECT];
	...
	else
	{
		...
		if(!nblocks)
		{
			...
			/* these changes will be written later, depending on inode_dep (set) */
			f->f_inode.i_blocks += object->blocksize / 512;
			f->f_inode.i_block[EXT2_DINDIRECT] = dindirect_number;
		}
		else
		{
			dindirect_number = f->f_inode.i_block[EXT2_DINDIRECT];
                                        ...
                                        chdesc_t ** tail, chdesc_pass_set_t * befores)
{
	...
	if (basep % object->blocksize + exists_rec_len_actual + new_rec_len_actual <= object->blocksize) {
		...
		dirblock = CALL(info->ubd, read_block, blockno, info->lfs.blocksize);
		if (!dirblock)
			return -1;
...
static int ext2_insert_dirent_set(LFS_t * object, ext2_fdesc_t * parent, EXT2_Dir_entry_t * new_dirent, chdesc_t ** tail, chdesc_pass_set_t * befores)
{
	...
	block = CALL(info->ubd, synthetic_read_block, new_block, info->lfs.blocksize);
	if (block == NULL)
		return -ENOSPC;
	...
	while(block_group != ( (*ino) / info->super->s_inodes_per_group) || firstrun) {
		if(info->inode_gdesc != block_group || info->inode_cache == NULL) {
			if (info->inode_cache != NULL)
				bdesc_release(&info->inode_cache);	
			info->inode_gdesc = block_group;
			bitmap = CALL(info->ubd, read_block, info->groups[block_group].bg_inode_bitmap, info->lfs.blocksize);
			if (!bitmap)
				return -ENOSPC;
			bdesc_retain(bitmap);
			bitmap->flags |= BDESC_FLAG_BITMAP;
                                    ...
                                    inode_t * new_ino, chdesc_t ** head)
{
	...
	if (!ln) {
		...
		if (type == TYPE_SYMLINK) {
			...
			dirblock_bdesc = CALL(info->ubd, synthetic_read_block, dirblock_no, info->lfs.blocksize);
...
static uint32_t ext2_erase_block_ptr(LFS_t * object, EXT2_inode_t * inode, chdesc_t ** head)
{
	...
	{
		blocknum -= EXT2_NDIRECT;
		block_desc = (CALL(info->ubd, read_block, inode->i_block[EXT2_INDIRECT], info->lfs.blocksize));
		if (!block_desc)
			return INVALID_BLOCK;
	...
	{
		blocknum -= (EXT2_NDIRECT + n_per_block);
		block_desc = (CALL(info->ubd, read_block, inode->i_block[EXT2_DINDIRECT], info->lfs.blocksize));
		if (!block_desc)
			return INVALID_BLOCK;
		block_nums = (uint32_t *)block_desc->data;
		indir_ptr = block_nums[blocknum / n_per_block];
		double_block_desc = CALL(info->ubd, read_block, indir_ptr, info->lfs.blocksize);
		if (!block_desc)
			return INVALID_BLOCK;
...
static int ext2_delete_dirent(LFS_t * object, ext2_fdesc_t * dir_file, uint32_t basep, uint32_t prev_basep, chdesc_t ** head)
{
	...
	if(basep % object->blocksize == 0) {
		...
		if (basep_blockno == INVALID_BLOCK)
			...
			return -1;
		dirblock = CALL(info->ubd, read_block, basep_blockno, info->lfs.blocksize);
		if(!dirblock)
			return -1;
		len = *((uint16_t *)(dirblock->data + (sizeof(inode_t))));
		...
		return CALL(info->ubd, write_block, dirblock, basep_blockno);
	}
	...
	//if deleting in the middle of a block, increase length of previous dirent
		...
		if (prev_basep_blockno == INVALID_BLOCK)
			return -1;
		dirblock = CALL(info->ubd, read_block, prev_basep_blockno, info->lfs.blocksize);
		if(!dirblock)
			return -1;

		//get the length of the deleted dirent

		len = *((uint16_t *) (dirblock->data + basep % object->blocksize + (sizeof(inode_t))));

		//get the length of the previous dirent:
		len += basep - prev_basep;
		
		//update the length of the previous dirent:
		if ((r = chdesc_create_byte(dirblock, info->ubd, ((prev_basep + 4) % object->blocksize), sizeof(len), (void *) &len, head) < 0))
			return r;
		KFS_DEBUG_SEND(KDB_MODULE_INFO, KDB_INFO_CHDESC_LABEL, *head, "delete dirent");
	
		return CALL(info->ubd, write_block, dirblock, prev_basep_blockno);
	}
}
	...
	if (id == KFS_FEATURE_SIZE) {
		...
		*((uint32_t *) data) = f->f_inode.i_atime;
		...
		if (size < EXT2_N_BLOCKS * sizeof(uint32_t))
			...
			symlink_block = CALL(info->ubd, read_block, f->f_inode.i_block[0], info->lfs.blocksize);
			if (!symlink_block)
				return -1;
			memcpy(data, symlink_block->data, f->f_inode.i_size);
		}	
	}
	else
		return -EINVAL;
...
static int ext2_write_slow_symlink(LFS_t * object, ext2_fdesc_t * f, char * name, uint32_t name_len, chdesc_t ** head)
{
	...
	new_block = CALL(info->ubd, synthetic_read_block, new_block_no, info->lfs.blocksize);
	if (!new_block)
		return -1;
...
static int ext2_get_inode(ext2_info_t * info, inode_t ino, EXT2_inode_t * inode)
{
	...
	bdesc = CALL(info->ubd, read_block, block, info->lfs.blocksize);
	if(!bdesc)
		return -EINVAL;
	offset &= info->lfs.blocksize - 1;
	memcpy(inode, bdesc->data + offset, sizeof(EXT2_inode_t));
...
int ext2_write_inode_set(struct ext2_info * info, inode_t ino, EXT2_inode_t * inode, chdesc_t ** tail, chdesc_pass_set_t * befores)
{
	...
	if(!bdesc)
		return -ENOENT;
	offset &= info->lfs.blocksize - 1;
	r = chdesc_create_diff_set(bdesc, info->ubd, offset, sizeof(EXT2_inode_t), &bdesc->data[offset], inode, tail, befores);
	if (r < 0)
		return r;
	...
	if (*tail && r > 0)
	{
		...
		r = CALL(info->ubd, write_block, bdesc, block);
	}
...
static int ext2_super_report(LFS_t * lfs, uint32_t group, int32_t blocks, int32_t inodes, int32_t dirs)
{
	...
	r = chdesc_create_diff(info->super_cache, info->ubd, 1024, 12 * sizeof(uint32_t),
	                       info->super_cache->data + 1024, info->super, &head);
	if (r < 0)
		return r;
	//chdesc_create_diff() returns 0 for "no change"
	if (head && r > 0)
	...
	r = chdesc_create_diff(info->gdescs[group_bdesc], info->ubd,
	                       group_offset, sizeof(EXT2_group_desc_t),
	                       info->gdescs[group_bdesc]->data + group_offset,
...
static int ext2_load_super(LFS_t * lfs)
{
	...
	info->super_cache = CALL(info->ubd, read_block, SUPER_BLOCKNO, info->ubd->blocksize);
	if (info->super_cache == NULL)
	{
	...
	memcpy(info->super, info->super_cache->data + 1024, sizeof(struct EXT2_Super));
	...
	for(i = 0; i < ngroupblocks; i++) {
		info->gdescs[i] = CALL(info->ubd, read_block, GDESC_BLOCKNO(i), info->lfs.blocksize);
		if(!info->gdescs[i])
			goto wb_fail2;
