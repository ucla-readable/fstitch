...
int chdesc_detach_dependencies(chdesc_t * chdesc)
{
	...
	assert(!chdesc->dependencies->next);
	return 0;
}
...
int chdesc_detach_dependents(chdesc_t * chdesc)
{
	...
	while((*scan)->desc != head)
	{
		...
		*head->dependents_tail = meta;
		head->dependents_tail = &meta->next;
