...
int ufs_read_inode(struct ufs_info * info, uint32_t num, struct UFS_dinode * inode)
{
	...
	inode_table = CALL(info->ubd, read_block, fragno, info->lfs.blocksize);
	if (!inode_table)
		return -ENOENT;
	wanted = (struct UFS_dinode *) (inode_table->data);
	wanted += frag_off;
...
int ufs_write_inode(struct ufs_info * info, uint32_t num, struct UFS_dinode inode, chdesc_t ** head)
{
	...
	r = chdesc_create_diff(inode_table, info->ubd, offset, sizeof(struct UFS_dinode), &inode_table->data[offset], &inode, head);
	if (r < 0)
		return r;
...
uint16_t ufs_read_fbp(struct ufs_info * info, uint32_t num)
{
	...
	ptr = ((uint32_t *) block->data) + (offset % super->fs_fsize) / 4;
...
int ufs_write_fragment_bitmap(struct ufs_info * info, uint32_t num, bool value, chdesc_t ** head)
{
	...
	cgblock = CALL(info->ubd, read_block, blockno, info->lfs.blocksize);
	if (!cgblock)
		return -ENOENT;
		*/
