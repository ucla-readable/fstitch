...
#define MAXOPEN		1024
#define FILEVA		(DISKMAP + DISKSIZE)
...
serve_open(envid_t envid, struct Fsreq_open* rq)
{
	...
	// Open the file
	if ((r = file_open(path, &f, rq->req_omode)) < 0) {
		if (debug)
...
serve(void)
{
	...
	while (1) {
		...
		case FSREQ_SYNC:
			serve_sync(whom);
			break;
		case FSREQ_AVAIL_SPACE:
			serve_avail_space(whom);
			break;
		case FSREQ_SHUTDOWN:
			serve_shutdown(whom);
			break;
